...
impl AvailableResourcePart {
        ...
        F: Fn(ItemEditorInteractionMessage) -> InteractionMessage + 'static + Copy,
    {
        let part_contents_col = Column::new()
            .push(Text::new(&self.part.name).font(SOURCE_HAN_SANS).size(16))
            .spacing(10);
        ...
        F: Fn(ItemEditorInteractionMessage) -> InteractionMessage + 'static + Copy,
    {
            ...
            match self.parts_tab_type {
                AvailablePartType::Parts => {
                    ...
                    let show_all_parts_checkbox =
                        Checkbox::new(self.show_all_available_parts, t!("inventory.available.show_all"), move |c| {
                            interaction_message(
        ...
        if let Some(available_parts) = &available_parts {
            ...
            let search_placeholder = match self.parts_tab_type {
                AvailablePartType::Parts => t!("inventory.available.search_parts", amount = &amount.to_string()),
                AvailablePartType::Anointments => {
                    t!("inventory.available.search_anointments", amount = &amount.to_string())
                }
            };
        ...
        if let Some(available_parts) = &available_parts {
            ...
            available_parts_column = available_parts_column.push(
                Container::new(
                    Text::new(t!("inventory.available.no_available"))
                        .font(SOURCE_HAN_SANS)
                        .size(17)
                        .color(Color::from_rgb8(220, 220, 220)),
                )
